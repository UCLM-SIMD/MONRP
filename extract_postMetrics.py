...
# agemoea2 and ctaea
# [True, False] # False for Feasibiliy first. true for repair per iteration
repair_deps = [True]
...
# number of solutions to choose from final NDS in each algorithm to compute metrics
subset_size = [10]
sss_type = [0]  # 0 for greedy hv
sss_per_iteration = [True]  # [True] # [True, False]
# [100, 200, 500, 700, 1000 #[, 2000, 3000] # 2000 and 3000 not in nsgaii (too slow)
population_size = [700]
...
def get_genetic_uids(name: str, d: str) -> List[str]:
    uids_list = []
...
def get_agemoea_uids(d: str) -> List[str]:
    uids_list = []
...
def get_ctaea_uids(d: str) -> List[str]:
    uids_list = []
...
def get_grasp_uids(d: str) -> List[str]:
    uids_list = []
...
def get_umda_uids(d: str) -> List[str]:
    uids_list = []
...
def get_random_uids(d: str) -> List[str]:
    uids_list = []
...
def get_pbil_uids(d: str) -> List[str]:
    uids_list = []
...
def get_mimic_uids(d: str) -> List[str]:
    uids_list = []
...
def get_feda_uids(d: str) -> List[str]:
    uids_list = []
...
if __name__ == '__main__':
    ...
    for data in dataset:
        ...
        reference_pareto, files_uid = construct_store_reference_pareto(
            files_uid)
    ...
    print(
        f"File list {container_name} created to be used from analisis notebook")
