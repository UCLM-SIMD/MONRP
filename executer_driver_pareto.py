...
if(params[0] == "genetic"):
    ...
    dataset_name, seed, pop_length, max_gens, evaluations, sel_scheme, selection_candidates, cross_scheme, cross_prob, mut_scheme, mut_prob, repl_scheme = \
    ...
    # algorithmtype algorithm dataset seed iterations solutions_per_iteration evaluations init_type local_search_type path_relinking_mode
    ...
    if(params[1] == "umda"):
        ...
        # algorithmtype algorithm dataset seed numpop gens evaluations selinds
        algorithm_model = UMDAAlgorithm

        algorithm_name, dataset_name, seed, numpop, gens, evaluations, selinds = \
            [str(params[1]), str(params[2]), int(params[3]),
        ...
        algorithm = algorithm_model(dataset_name=dataset_name, population_length=numpop,
                                    max_generations=gens, max_evaluations=evaluations, selected_individuals=selinds, random_seed=seed)
        filepath = "output/paretos/pareto-umda-"+algorithm.file

    elif(params[1] == "pbil"):
        # -c eda pbil s1 5 100 300 10000 0.1 0.1 0.1
        # algorithmtype algorithm dataset seed numpop gens evaluations lr mutprob mutshift
