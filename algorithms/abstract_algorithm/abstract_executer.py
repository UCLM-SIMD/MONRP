...
class AbstractExecuter(ABC):
    ...
    def execute(self, output_folder: str) -> None:
        ...
        for it in range(0, self.executions):
            ...
            # get a list with pareto points
            pareto = self.get_pareto(result['population'])
        ...
        unique_id = ''.join(str(c)
                            for c in self.algorithm.config_dictionary.values())
    ...
    def get_metrics_fields(self, result, repetition):
        ...
        hv = metrics.calculate_hypervolume(
            result["population"], ref_x=1.1, ref_y=1.1)
